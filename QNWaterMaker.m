...
//
        ret =  avcodec_parameters_from_context(par,
                                            out_stream->codec);
//
//        if (ret < 0) {
...
//        while (av_audio_fifo_size(fifo) < output_frame_size  && filt_frame) {
            if(filt_frame)
            {
...
static int filter_encode_write_frame(AVFrame *frame, unsigned int stream_index)
{
    ...
    while (1) {
        ...
        //[aac @ 0x1094ec00] more samples than frame size (avcodec_encode_audio2)
        //ffmpeg transcode_aac.c中有讲解 mp4 与 aac确实有点不同
        //http://blog.csdn.net/ren65432/article/details/40682197?_t_t_t=0.6326977820135653
